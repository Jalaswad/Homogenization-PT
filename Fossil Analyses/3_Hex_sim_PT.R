...
#--------------------------------------------------------------------------------------
Genus_occs_pt <-ddply(data.j, c("interval.ma", "cell", "accepted_name"),
                       function(df)c(length(df$accepted_name)))
names(Genus_occs_pt) <- c("interval.ma","cell","accepted_name", "occs")

Changhsingian <- subset(Genus_occs_pt, interval.ma=="252.17")
Induan <-subset(Genus_occs_pt,interval.ma=="251.2")
...
#split.genusoccs   <- split(Genus_occs_pt, Genus_occs_pt$interval.ma) #split for each interval
...
Sim.i<- subset(Sim.i, Occs.x >20 & Occs.y >20)
...
table(Sim.c$CutDist)
table(Sim.i$CutDist)
Sum.Cz$bubbles <- c(7,4,8,7,12,8,2,4,2,1) 
Sum.Ja$bubbles <- c(7,4,8,7,12,8,2,4,2,1)
#bubbles = number of cell-pair comparisons. found in Sim dataframes using table() above
...
Sum.i.Cz$bubbles <- c(11,2,16,9,10,20,9,1) 
Sum.i.Ja$bubbles <- c(11,2,16,9,10,20,9,1) 
...
plot2.g <- Sum.Cz.2 %>%
  ...
  scale_x_continuous(limits=c(1,9), breaks=c(1, 2, 3, 4, 5, 6,
                     ...
                     labels= as.character(c("2000", "4000", "6000",
                                            ...
                                            "14000", "16000", "18000")))+
  geom_errorbar(aes(ymin=pmax(avg-sdev,0), ymax=avg+sdev), width=0.05,size=1, alpha=0.7) + 
  theme(plot.margin = unit(c( t= 2, r=2, b=2,l=1), "cm"))+
  theme(axis.title.x=element_blank(), axis.title.y=element_blank(),
        plot.title = element_text(size=35), legend.position=c(.8, .9))+
  guides(size = "none")+ #remove legend for bubbles
  labs(title = bquote(~bold('B')~    ' Czekanowski'))
#~bold makes the next part bold. Each section has a ~ before the text
plot2.g

ggsave("Czek.pt.oct2023.pdf", plot = plot2.g,
       height=10, width=17, device="pdf", dpi=500) 

ggsave("Jacc.pt.oct2023.pdf", plot=plot1.g,
       height=10, width=17, device="pdf", dpi=500)
